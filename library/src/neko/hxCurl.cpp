...
static size_t writeCallbackHelper(void *ptr, size_t size, size_t nmemb, void *usrdata)
...
static value setWriteCallbackHandler( value curl, value hfunc, value bfunc, value data )
{	// Sets a callback to handle the data recieved from the net
	...
	// *** * *** Header //
	WriteCallbackData *hlump = malloc(sizeof(WriteCallbackData));
	...
	*hlump->data = data;
	...
	WriteCallbackData *blump = malloc(sizeof(WriteCallbackData));
	...
	*blump->data = data;
	...
	// Tell Curl about it
	curl_easy_setopt(val_data(curl), CURLOPT_WRITEFUNCTION, writeCallbackHelper);
...
value makeRequest(value curl_handle)
...
value setUserAgent(value curl_handle, value ua)
...
// FIXME: Second parameter is ignored
value enableCookies( value curl_handle, value cookie_file )
